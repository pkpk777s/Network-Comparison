...
############################################################
def plot_training_curves(train_losses, train_accs, val_accs, out_dir):
    """
    ...
    # Add marker for min loss
    min_loss_epoch = np.argmin(train_losses) + 1
    ...
    # Plot accuracy
    plt.subplot(1, 2, 2)
    plt.plot(epochs, train_accs, 'b-', marker='o', label='Training')
    plt.plot(epochs, val_accs, 'r-', marker='s', label='Validation')
    ...
    # Add markers for max accuracies
    max_train_acc_epoch = np.argmax(train_accs) + 1
    ...
    plt.annotate(f'Max: {max_train_acc:.2f}%', 
                ...
                xytext=(max_train_acc_epoch+0.3, max_train_acc-5),
                arrowprops=dict(facecolor='black', shrink=0.05))
    
    plt.annotate(f'Max: {max_val_acc:.2f}%', 
                xy=(max_val_acc_epoch, max_val_acc), 
                xytext=(max_val_acc_epoch+0.3, max_val_acc-5),
                arrowprops=dict(facecolor='black', shrink=0.05))
    ...
    metrics_path = os.path.join(out_dir, 'resnet_metrics.npz')
...
def train(model, train_loader, criterion, optimizer, device, num_epochs, transform_fn):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in pbar:
            ...
            pbar.set_postfix({"Loss": f"{current_loss:.4f}", "Acc": f"{batch_acc:.2f}%"})
        ...
        val_acc = evaluate_validation(model, train_loader.dataset, device, transform_fn)
    ...
    return train_losses, train_accs, val_accs, best_state

@torch.no_grad()
def evaluate_validation(model, dataset, device, transform_fn):
    """
    Evaluate on validation set during training
    """
    eval_loader = FaceDataLoader(dataset, batch_size=16, shuffle=False)
    eval_loader.set("validation")
    model.eval()
    total = 0
    correct = 0
...
def parse_args():
    ...
    parser.add_argument("--lr", type=float, default=0.001, 
                        help="Learning rate.")
    parser.add_argument("--out-dir", type=str, default="../ResNet_result",
...
if __name__ == "__main__":
    ...
    train_losses, train_accs, val_accs, best_state = train(model, train_loader, criterion, optimizer, device, 
                                    num_epochs=args.epochs, transform_fn=transform_fn)
    
    # Plot training curves
    plot_training_curves(train_losses, train_accs, val_accs, out_dir)
